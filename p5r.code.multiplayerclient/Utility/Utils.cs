...
internal class Utils
{
    ...
    internal static bool Initialise(ILogger logger, Config config, IModLoader modLoader)
    {
        ...
        {
            _logger.WriteLine($"Unable to get controller for Reloaded SigScan Library, stuff won't work :(");
            return false;
        }
    ...
    internal static void SigScan(string pattern, string name, Action<nint> action)
    {
        _startupScanner.AddMainModuleScan(pattern, result =>
        {
            if (!result.Found)
            {
                _logger.WriteLine($"Unable to find {name}, stuff won't work :(");
                return;
            }
            _logger.WriteLine($"Found {name} at 0x{result.Offset + BaseAddress:X}");
